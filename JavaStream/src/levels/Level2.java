...
public class Level2 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("TO DO 2: Retourner la somme des salaires de tous les enseignants Flutter (hint: mapToInt) ");
        long sum = teachers.stream().filter(teacher -> teacher.getSubject()
                .equals(Subject.FLUTTER)).mapToInt(Teacher::getSalary).sum();
        ...
        double average = teachers.stream().filter(teacher -> teacher.getName().startsWith("a"))
                .mapToDouble(Teacher::getSalary).average().getAsDouble();
        ...
        List<Teacher> teachers2 = teachers.stream().filter(teacher -> teacher.getName().startsWith("s")).toList();
        ...
        System.out.println(" TO DO 6: Afficher le premier enseignant Unity le nom commence avec g " +
                        "avec 2 manières différentes ");
        System.out.println("First way ");
        teachers.stream().filter(teacher -> teacher.getName().startsWith("g")).filter(teacher -> teacher.getSubject()
                        .equals(Subject.UNITY))
                .findFirst().ifPresent(System.out::println);


        System.out.println("Second Way");
        teachers.stream().filter(teacher -> teacher.getName()
                        .startsWith("g")).filter(teacher -> teacher.getSubject().equals(Subject.UNITY))
                .limit(1).forEach(System.out::println);



        System.out.println("TO DO 7: Afficher le deuxième enseignant dont le nom commence avec s");
        teachers.stream().filter(teacher -> teacher.getName().startsWith("s")).skip(1).
                findFirst().ifPresent(System.out::println);/* TO DO 8 */;
